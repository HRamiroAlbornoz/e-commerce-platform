...
describe('POST /api/orders/cancel', () => {
  it('cancela una orden pendiente propia y devuelve el stock de cada item (F7.6, F7.7)', async () => {
  ...
  it('una orden que no existe rechaza con ORDER_NOT_FOUND', async () => {
    ...
    const { res, status, json } = buildResponse();
    await handler(buildRequest({ orderId: randomUUID() }), res);

    expect(status).toHaveBeenCalledWith(404);
    expect(json).toHaveBeenCalledWith(expect.objectContaining({ code: 'ORDER_NOT_FOUND' }));
  });

  it('la orden de otro usuario tambien rechaza con ORDER_NOT_FOUND, sin distinguirla de una inexistente (F7.2)', async () => {
  ...
  it.each(['processing', 'completed', 'cancelled'])(
    'una orden en estado %s no se puede cancelar (F7.8)',
    async (status) => {
      const uid = randomUUID();
      const productId = randomUUID();
      const orderId = randomUUID();
  ...
  it('si el producto ya no existe, igual cancela la orden sin restituir ese item', async () => {
    ...
    const stillExistingProductId = randomUUID();
    const orderId = randomUUID();
  ...
  it('cancelar dos veces la misma orden no devuelve el stock dos veces', async () => {
    const uid = randomUUID();
    const productId = randomUUID();
    const orderId = randomUUID();
    await seedProduct(productId, { stock: 3 });
    ...
    await handler(buildRequest({ orderId }), first.res);
