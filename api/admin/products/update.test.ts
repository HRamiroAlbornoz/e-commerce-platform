...
const { default: handler } = await import('./update.js');
...
describe('POST /api/admin/products/update', () => {
  it('un admin edita el precio: lo redondea a centavos y actualiza updatedAt', async () => {
    const productId = randomUUID();
    await seedProduct(productId, { price: 10000 });
    mockAuthenticatedAs('admin');

    const { res, status, json } = buildResponse();
    await handler(buildRequest({ productId, changes: { price: 12345.678 } }), res);

    expect(status).toHaveBeenCalledWith(200);
    expect(json).toHaveBeenCalledWith({ productId });

    const productSnap = await adminDb.doc(`products/${productId}`).get();
    expect(productSnap.data()).toMatchObject({ price: 12345.68 });
  });

  it('editar el nombre recalcula nameLower en el servidor', async () => {
    const productId = randomUUID();
    await seedProduct(productId);
    mockAuthenticatedAs('admin');

    const { res, status } = buildResponse();
    await handler(buildRequest({ productId, changes: { name: 'Teclado Núcleo' } }), res);
  ...
  it('retirar del catalogo es un patch parcial con isActive:false', async () => {
    const productId = randomUUID();
    await seedProduct(productId, { isActive: true });
    mockAuthenticatedAs('admin');

    const { res, status } = buildResponse();
    await handler(buildRequest({ productId, changes: { isActive: false } }), res);

    expect(status).toHaveBeenCalledWith(200);
    const productSnap = await adminDb.doc(`products/${productId}`).get();
    expect(productSnap.data()).toMatchObject({ isActive: false, stock: 3 });
  });

  it('un customer no puede editar productos (403 FORBIDDEN)', async () => {
    const productId = randomUUID();
    await seedProduct(productId);
    mockAuthenticatedAs('customer');

    const { res, status, json } = buildResponse();
    await handler(buildRequest({ productId, changes: { stock: 5 } }), res);
  ...
  it('sin token valido, rechaza con 401', async () => {
    ...
    const { res, status, json } = buildResponse();
    await handler(buildRequest({ productId: randomUUID(), changes: { stock: 5 } }), res);

    expect(status).toHaveBeenCalledWith(401);
    expect(json).toHaveBeenCalledWith(expect.objectContaining({ code: 'UNAUTHENTICATED' }));
  });

  it('con "changes" vacio, rechaza con 400 en vez de escribir un no-op', async () => {
    mockAuthenticatedAs('admin');

    const { res, status, json } = buildResponse();
    await handler(buildRequest({ productId: randomUUID(), changes: {} }), res);
  ...
  it('un productId con "/" se rechaza con 400 en vez de resolverse como un path de Firestore distinto', async () => {
    ...
    const { res, status, json } = buildResponse();
    await handler(
      buildRequest({ productId: `${randomUUID()}/reviews/otro-uid`, changes: { stock: 5 } }),
      res,
    );
