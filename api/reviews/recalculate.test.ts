...
const { default: handler } = await import('./recalculate.js');
...
async function seedReview(productId: string, rating: number): Promise<void> {
  await adminDb.doc(`products/${productId}/reviews/${randomUUID()}`).set({
    userId: randomUUID(),
    displayName: 'Autor',
    rating,
    comment: 'Comentario',
...
describe('POST /api/reviews/recalculate', () => {
  it('recalcula el promedio y la cantidad a partir de las reseñas reales', async () => {
    const productId = randomUUID();
    await seedProduct(productId);
    await seedReview(productId, 5);
    await seedReview(productId, 4);
    await seedReview(productId, 4);
    ...
    expect(json).toHaveBeenCalledWith({ productId, ratingAverage: 4.3, ratingCount: 3 });

    const productSnap = await adminDb.doc(`products/${productId}`).get();
    expect(productSnap.data()).toMatchObject({ ratingAverage: 4.3, ratingCount: 3 });
  });

  it('un producto sin reseñas normaliza el promedio a 0, no a null ni NaN (F8.6)', async () => {
    const productId = randomUUID();
    await seedProduct(productId);
    mockAuthenticatedAs(randomUUID());
    ...
    expect(status).toHaveBeenCalledWith(200);
    expect(json).toHaveBeenCalledWith({ productId, ratingAverage: 0, ratingCount: 0 });

    const productSnap = await adminDb.doc(`products/${productId}`).get();
    expect(productSnap.data()).toMatchObject({ ratingAverage: 0, ratingCount: 0 });
  });

  it('un producto inexistente rechaza con PRODUCT_NOT_FOUND', async () => {
    mockAuthenticatedAs(randomUUID());
  ...
  it('un productId con "/" se rechaza con 400 en vez de resolverse como un path de Firestore distinto', async () => {
    ...
    const { res, status, json } = buildResponse();
    await handler(buildRequest({ productId: `${productId}/reviews/otro-uid` }), res);
